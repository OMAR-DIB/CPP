...
void loadInput(char** av, Container& data) {
    for (int i = 1; av[i]; ++i) {
        ...
        while (ss >> number) { // is a loop that extracts data from the stringstream ss
...
void ensureUnique(const Container& data) {
    ...
    for (it = data.begin(); it != data.end(); ++it) {
        if (!seen.insert(*it).second) { // second : It's false if the element was not inserted because a duplicate was already present in the set.
...
std::vector<int> adjustJacobIndices(const std::vector<int>& jacob, int pendSize) {
    ...
    for (size_t i = 0; i < jacob.size(); i++) {
        int j = jacob[i];
        while (j > 1) {
            if (used.find(j) == used.end()) { // if j is not in used
                order.push_back(j);
                used.insert(j);
            }
            j--;
    ...
    for (int x = pendSize; (int)order.size() < pendSize; x--) {
        if (used.find(x) == used.end()) { // if x is not in used
            order.push_back(x);
            used.insert(x);
        }
    }
...
Container fordJohnsonSort(Container data) {
    ...
    if (hasOdd) pend.push_back(data.back());
    ...
    if (hasOdd) alignedPend.push_back(pend.back());
